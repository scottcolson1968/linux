 ...
 * - simple_strtoull() is not overflow-safe and it is not recommended;
 * - kstrtoull() is safe, but requires termination and does not provide a
 *   pointer to the end of the string (it would required a copy to a temporary
 *   buffer).
 *
 * This implementation may need to change to reuse those functions if their
 * limitations are addressed or if a new suitable helper is added to the kernel.
 *
 * Returns:
 ...
 * Positive values greater than S64_MAX are not flagged as errors, but
 * returned as-is. Negative values less than S64_MIN are flagged as errors.
 *
 * Returns:
